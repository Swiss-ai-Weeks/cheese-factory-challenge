...
TOTAL=$(
  "$PROJECT_ROOT/.venv/bin/python" - "$PROJECT_ROOT/data/processed" "$SPLITS" "$BINS" "$SOURCE_MANIFEST" <<'PY'
import csv
import sys
...
source_manifest = sys.argv[4].strip()
...
if source_manifest:
    ...
    print(sum(
        1 for row in rows
        if (not splits or row["split"] in splits)
...
for ((offset = START_OFFSET; offset < TOTAL; offset += CHUNK_SIZE)); do
  ...
  CHEESE_CAPTURE_DATASET="$DATASET" \
  CHEESE_CAPTURE_SOURCE_MANIFEST="$SOURCE_MANIFEST" \
